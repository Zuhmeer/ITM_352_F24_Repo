...
def predict_match():
    ...
    if not home_team or not away_team:
        return render_template('index.html', error="Please select both teams.")

    # Simulate a prediction logic
    predicted_winner = random.choice([home_team, away_team])  # Random for now, replace with actual logic
    win_probability = random.random()  # Random probability for demonstration

    return render_template('index.html', home_team=home_team, away_team=away_team,
