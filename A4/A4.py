...
def load_nfl_data():
    ...
    lambda team: next((div for div, teams in divisions.items() if team in teams), None)
)
...
def standings2():
    if nfl_data and 'standings2' in nfl_data and 'teams' in nfl_data:
...
#@app.route('/pastgames2', methods=['GET'])
#def past_games():
#    if nfl_data and 'games' in nfl_data:
#       past_games = nfl_data['games'][nfl_data['games']['result'].notna()]
#        return jsonify(past_games[['home_team', 'away_team', 'home_score', 'away_score', 'result']].to_dict(orient='records'))
...
#    if nfl_data and 'games' in nfl_data:
#        past_games = nfl_data['games'][nfl_data['games']['result'].notna()]
#        return render_template('pastgames2.html', games=past_games.to_dict(orient='records'))
#    else:
#        return jsonify({"error": "Past games data not available."}), 500