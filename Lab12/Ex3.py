...
print(f"Opening {url}...")
imt_html = urllib.request.urlopen(url)
html_to_parse = BeautifulSoup(imt_html, "html.parser")
...
# Create a list of people found 
itm_people = []
for element in list_of_people:
    itm_people.append(element.text)
    print(element.text)

print(f"Number of ITM faculty: ", len(itm_people))